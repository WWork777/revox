import Image from 'next/image'
import './Team.scss'
import Link from 'next/link'

export default function Team(){

    const team = [
        {id: 1, img: "Данил.jpg", name: "Данил Мамзер", specialization: "Педагог по вокалу, звукорежиссёр", text: "Помогает ученикам раскрыть голос и&nbsp;записать треки, звучащие как&nbsp;на&nbsp;радио. Более 10&nbsp;лет опыта работы со&nbsp;звуком.", vk: 'https://vk.com/danyaveinz', tg: 'https://t.me/danilvox', inst: 'https://www.instagram.com/spectroveinz/'},
        {id: 2, img: "Ольга.jpg", name: "Ольга Доронина", specialization: "Педагог по вокалу, руководитель хора", text: "Преподавательский опыт более&nbsp;4&nbsp;лет. Обучалась по&nbsp;зарубежным методикам. Научит вокальным техникам с&nbsp;позитивом и&nbsp;заботой о&nbsp;голосе", vk: 'https://vk.com/doroninaolya', tg: 'https://t.me/doronina_olga', inst: 'https://www.instagram.com/doroninaolya_/'},
        {id: 3, img: "Алексей.jpg", name: "Алексей Петров", specialization: "Экстрим-вокал, электрогитара", text: "Преподаёт экстремальный вокал и&nbsp;игру на&nbsp;электрогитаре. Учит&nbsp;не&nbsp;только звучать мощно, но&nbsp;и&nbsp;делать это безопасно.", vk: 'https://vk.com/ghxst3r', tg: 'https://t.me/ghxst3r', inst: 'https://www.instagram.com/ghxst3r/'},
        {id: 4, img: "Анастасия.jpg", name: "Анастасия Борисенко", specialization: "Педагог по вокалу", text: "Находит подход к&nbsp;каждому ученику, помогает раскрыться, обрести уверенность и&nbsp;любовь к&nbsp;своему&nbsp;голосу.", vk: 'https://vk.com/abborisenko', tg: 'https://t.me/abborisenko', inst: 'https://www.instagram.com/abborisenko/'},
    ]

    return(
        <>
            <div id='team' className='home-block team'>
                <div className='team-heading'>
                    <h2>Наша команда</h2>
                    <p>Люди, благодаря которым здесь хочется расти и&nbsp;творить<br></br>
                    Молодые педагоги и звукорежиссёры, которые любят музыку и умеют вдохновлять</p>
                </div>
                <div className='team-container'>
                    {team.map(item => (
                        <div className='team-card' key={item.id}>
                            <Image src={`/Home/Team/${item.img}`} width={700} height={500} alt={`Преподаватель вокальной студии VOX в Кемерове ${item.name}`}/>
                            <div className='team-card-info'>
                                <div className='team-card-links'>
                                    <Link href={item.vk}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 50 50" fill="none">
                                        <g clipPath="url(#clip0_320_270)">
                                        <path d="M26.0557 0C37.3573 0 43.0395 0.000132636 46.5195 3.50977C49.9995 7.01978 50 12.6723 50 23.9443V26.0557C50 37.3298 50.0295 42.9802 46.5195 46.4902C43.0095 49.9999 37.3573 50 26.0557 50H23.9736C12.6699 50 7.01974 50.0002 3.50977 46.4902C-0.000221893 42.9802 0 37.3298 0 26.0557V23.9443C0 12.6683 -0.000221893 7.01978 3.50977 3.50977C7.01974 -0.000184051 12.6699 6.26696e-10 23.9736 0H26.0557ZM8.44922 15.2295C8.71523 28.2275 15.558 36.0498 26.832 36.0498H27.4854V28.6143C31.5893 29.0303 34.6533 32.0938 35.9033 36.0498H41.8213C40.2154 30.1322 36.0518 26.8605 33.4639 25.6104C36.0518 24.0643 39.7092 20.3155 40.5713 15.2295H35.1895C34.0576 19.3635 30.6973 23.1118 27.4854 23.4678V15.2295H22.0117V29.6563C18.6817 28.8303 14.3372 24.7775 14.1592 15.2295H8.44922Z" fill="white"/>
                                        </g>
                                        <defs>
                                        <clipPath id="clip0_320_270">
                                        <rect width="50" height="50" fill="white"/>
                                        </clipPath>
                                        </defs>
                                        </svg>
                                    </Link>
                                    <Link href={item.tg}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 50 50" fill="none">
                                        <g clipPath="url(#clip0_320_269)">
                                        <path fillRule="evenodd" clipRule="evenodd" d="M50 25C50 38.8071 38.8071 50 25 50C11.1929 50 0 38.8071 0 25C0 11.1929 11.1929 0 25 0C38.8071 0 50 11.1929 50 25ZM25.8959 18.4561C23.4643 19.4675 18.6045 21.5608 11.3164 24.7361C10.133 25.2067 9.51304 25.6671 9.4566 26.1173C9.36122 26.8781 10.314 27.1777 11.6114 27.5857C11.7879 27.6412 11.9707 27.6987 12.1582 27.7596C13.4347 28.1745 15.1517 28.6599 16.0443 28.6792C16.854 28.6967 17.7578 28.3629 18.7555 27.6778C25.5649 23.0813 29.08 20.7579 29.3006 20.7078C29.4563 20.6725 29.6721 20.6281 29.8183 20.758C29.9644 20.8879 29.9501 21.134 29.9346 21.2C29.8402 21.6024 26.1003 25.0794 24.1648 26.8787C23.5615 27.4396 23.1335 27.8375 23.046 27.9284C22.85 28.132 22.6503 28.3245 22.4583 28.5096C21.2724 29.6528 20.3831 30.51 22.5075 31.91C23.5284 32.5827 24.3453 33.139 25.1603 33.6941C26.0504 34.3002 26.9381 34.9047 28.0867 35.6576C28.3793 35.8495 28.6588 36.0487 28.931 36.2428C29.9668 36.9812 30.8974 37.6446 32.0471 37.5388C32.7151 37.4773 33.4051 36.8492 33.7556 34.9757C34.5838 30.5482 36.2117 20.9551 36.588 17.002C36.6209 16.6557 36.5794 16.2124 36.5461 16.0179C36.5129 15.8233 36.4433 15.5461 36.1903 15.3408C35.8908 15.0978 35.4284 15.0465 35.2216 15.0502C34.2813 15.0667 32.8387 15.5684 25.8959 18.4561Z" fill="white"/>
                                        </g>
                                        <defs>
                                        <clipPath id="clip0_320_269">
                                        <rect width="50" height="50" fill="white"/>
                                        </clipPath>
                                        </defs>
                                        </svg>
                                    </Link>
                                    <Link href={item.inst}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 50 50" fill="none">
                                        <g clipPath="url(#clip0_320_268)">
                                        <path d="M25.0147 4.50283C31.6983 4.50283 32.4897 4.53214 35.1182 4.64935C37.5611 4.75679 38.8802 5.16703 39.7596 5.50889C40.9224 5.9582 41.7628 6.50518 42.6324 7.37449C43.5118 8.25357 44.0492 9.08381 44.4987 10.2461C44.8407 11.1252 45.2511 12.4536 45.3586 14.8857C45.4759 17.523 45.5052 18.3141 45.5052 24.9854C45.5052 31.6663 45.4759 32.4575 45.3586 35.085C45.2511 37.5269 44.8407 38.8455 44.4987 39.7246C44.0492 40.8869 43.5021 41.7269 42.6324 42.5962C41.753 43.4753 40.9224 44.0125 39.7596 44.4618C38.8802 44.8037 37.5513 45.2139 35.1182 45.3214C32.48 45.4386 31.6885 45.4679 25.0147 45.4679C18.3311 45.4679 17.5396 45.4386 14.9111 45.3214C12.4682 45.2139 11.1491 44.8037 10.2697 44.4618C9.1069 44.0125 8.26656 43.4655 7.39691 42.5962C6.51749 41.7171 5.98007 40.8869 5.53058 39.7246C5.18859 38.8455 4.77819 37.5171 4.67071 35.085C4.55345 32.4477 4.52414 31.6566 4.52414 24.9854C4.52414 18.3044 4.55345 17.5132 4.67071 14.8857C4.77819 12.4438 5.18859 11.1252 5.53058 10.2461C5.98007 9.08381 6.52726 8.2438 7.39691 7.37449C8.27633 6.49541 9.1069 5.9582 10.2697 5.50889C11.1491 5.16703 12.478 4.75679 14.9111 4.64935C17.5396 4.53214 18.3311 4.50283 25.0147 4.50283ZM25.0147 0C18.2236 0 17.3735 0.0293026 14.7059 0.146513C12.0481 0.263723 10.2208 0.693495 8.63787 1.30885C6.98652 1.95351 5.58921 2.80328 4.20168 4.20004C2.80438 5.58703 1.95427 6.98379 1.30936 8.62473C0.693766 10.2168 0.263826 12.0336 0.14657 14.6904C0.0293141 17.3667 0 18.2164 0 25.0049C0 31.7933 0.0293141 32.6431 0.14657 35.3096C0.263826 37.9664 0.693766 39.7929 1.30936 41.3753C1.95427 43.026 2.80438 44.4227 4.20168 45.8097C5.58921 47.1967 6.98652 48.0563 8.6281 48.6912C10.2208 49.3065 12.0383 49.7363 14.6961 49.8535C17.3637 49.9707 18.2138 50 25.0049 50C31.796 50 32.6461 49.9707 35.3137 49.8535C37.9715 49.7363 39.7987 49.3065 41.3817 48.6912C43.0233 48.0563 44.4206 47.1967 45.8081 45.8097C47.1956 44.4227 48.0555 43.026 48.6906 41.385C49.3062 39.7929 49.7362 37.9762 49.8534 35.3194C49.9707 32.6529 50 31.8031 50 25.0147C50 18.2262 49.9707 17.3764 49.8534 14.7099C49.7362 12.0531 49.3062 10.2266 48.6906 8.64427C48.075 6.98379 47.2249 5.58703 45.8276 4.20004C44.4401 2.81305 43.0428 1.95351 41.4012 1.31862C39.8085 0.703262 37.991 0.273491 35.3332 0.156281C32.6559 0.0293026 31.8057 0 25.0147 0Z" fill="white"/>
                                        <path d="M25 12.5C18.0989 12.5 12.5 18.0989 12.5 25C12.5 31.9011 18.0989 37.5 25 37.5C31.9011 37.5 37.5 31.9011 37.5 25C37.5 18.0989 31.9011 12.5 25 12.5ZM25 33.1084C20.5228 33.1084 16.8916 29.4772 16.8916 25C16.8916 20.5228 20.5228 16.8916 25 16.8916C29.4772 16.8916 33.1084 20.5228 33.1084 25C33.1084 29.4772 29.4772 33.1084 25 33.1084Z" fill="white"/>
                                        <path d="M40.625 11.7188C40.625 13.0166 39.5715 14.0625 38.2813 14.0625C36.9834 14.0625 35.9375 13.009 35.9375 11.7188C35.9375 10.4209 36.991 9.375 38.2813 9.375C39.5715 9.375 40.625 10.4285 40.625 11.7188Z" fill="white"/>
                                        </g>
                                        <defs>
                                        <clipPath id="clip0_320_268">
                                        <rect width="50" height="50" fill="white"/>
                                        </clipPath>
                                        </defs>
                                        </svg>
                                    </Link>
                                </div>
                                <div className='team-card-description'>
                                    <p className='team-card-name'>{item.name}</p>
                                    <p className='team-card-specialization'>{item.specialization}</p>
                                    <p className='team-card-text' dangerouslySetInnerHTML={{ __html: item.text }}></p>
                                </div>
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </>
    )
}